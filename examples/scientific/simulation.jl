...
# can be expensive. A schema is not a CPU or memory sandbox.
decay_tool = FunctionTool(
    name="predict_decay",
    description="Predict exponential decay over a bounded interval",
    parameters=Dict(
        "type" => "object",
        "properties" => Dict(
            "initial" => Dict("type" => "number", "minimum" => 0),
            "rate" => Dict("type" => "number", "minimum" => 0, "maximum" => 100),
            "duration" => Dict("type" => "number", "exclusiveMinimum" => 0, "maximum" => 10, "default" => 1.0),
        ),
        "required" => ["initial", "rate"],
    ),
)
function predict_decay(initial::Float64, rate::Float64; duration::Float64=1.0)
...
# Your code reads the numbers from the call's JSON input; a string where a number
# belongs is refused, not evaluated.
function number(input, key; default=nothing)
    value = get(input, key, default)
    value isa Real && !(value isa Bool) && isfinite(value) || throw(ArgumentError("$key must be a number"))
    return Float64(value)
end
decay_call = tool_call("decay-1", "predict_decay", Dict("initial" => 2, "rate" => 0.5))
input = decay_call.input
decay_summary = predict_decay(number(input, "initial"), number(input, "rate"); duration=number(input, "duration"; default=1.0))
decay_output = tool_result(decay_call, tool_content(decay_summary))
...
isapprox(decay_summary.final, analytic_final; atol=1e-9)
...
# to explain it.
#
# **Next:** [custom result types](../guides/extensions.md).
