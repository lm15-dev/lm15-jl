...
for (name, signature, explanation) in (
    ...
    (:LiveConfig, "LiveConfig(; model, system=nothing, tools=(), voice=nothing, input_format=nothing, output_format=nothing, extensions=nothing)", "Configure a live session. Tools may include ToolBindings but only their specifications are sent. Provider support determines voices, formats and extensions. Creating the value neither opens a socket nor runs tools."),
    ...
    (:send_tool_result!, "send_tool_result!(session, id, content)", "Send presentational content answering a live tool call. Use the call's ID. For an execute_tool result, pass result.id and result.content; do not nest ToolResultPart as live content."),
